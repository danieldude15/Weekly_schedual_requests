...
function died($msg){
    echo '<p>'.
    'אתר נמצא בבדיקות אם אינך מנהל האתר יש להתעלם מההערות הבאות<br>'
            . $msg."<br></p>";
}
...
function edit_add_emp($post,$conn){
$taz=$post["inerid"];
$first=$post["f_name"];
$last=$post["l_name"];
$pass=$post["newpass"];
...
$sql .= "WHERE `emp`.`inerid` = '".$post['inerid']."'";
 if (mysqli_query($conn, $sql)) {
    ...
    echo "what?";
    $sql = "INSERT INTO `emp` (`inerid`, `firstname_rash`, `lastname_rash`, `team`, `pass`, `misra`,`indexG`,`super`,`nightsuper`,`driver`,`religion`,`nights` ) VALUES ('$taz', '$first','$last','$team','$pass','$misra','$index','$super','$nightsuper','$driver','$religion','$nights')";
